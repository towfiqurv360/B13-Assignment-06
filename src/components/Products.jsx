          ...
          return (
            ...
            >
              <div className="flex justify-between items-start mb-6">
                <div className="p-4 bg-purple-50 rounded-2xl transition-colors group-hover:bg-purple-100">
                  <img src={product.icon} alt={product.name} className="w-12 h-12 object-contain" />
                </div>
                
                <span className={`badge border-none font-bold py-3 px-4 rounded-full text-xs uppercase tracking-wider ${
                  product.tagType === 'popular' ? 'bg-blue-50 text-blue-600' :
                  product.tagType === 'new' ? 'bg-green-50 text-green-600' :
